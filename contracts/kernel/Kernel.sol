...
import "contracts/kernel/common/Constants.sol";
import "contracts/kernel/common/Enum.sol";
...
contract Kernel is EIP712, Compatibility, KernelStorage {
    ...
    function validateUserOp(
        ...
        assembly {
            ...
            userOpSignature.offset := add(
                calldataload(add(userOpEndOffset, 0x120)),
                userOpEndOffset
            )
            userOpSignature.length := calldataload(
                sub(userOpSignature.offset, 0x20)
            )
        ...
        if (mode == 0x00000000) {
            ...
            ExecutionDetail storage detail = getKernelStorage().execution[
                bytes4(userOpCallData[0:4])
            ];
            ...
            userOpSignature = userOpSignature[4:];
            validationData = packValidationData(
                detail.validAfter,
                detail.validUntil
            );
            ...
            (validator, validationData, userOpSignature) = _approveValidator(
                bytes4(userOpCallData[0:4]),
                userOpSignature
            );
        } else {
        ...
        unchecked {
            ...
            validationData = _intersectValidationData(
                getKernelStorage().defaultValidator.validateSignature(
                    enableDigest,
                    signature[cursor:cursor + length]
                ),
                ValidationData.wrap(
                    uint256(bytes32(signature[4:36])) &
    ...
    function isValidSignature(
        ...
        ValidationData validationData = getKernelStorage()
            .defaultValidator
    ...
    function _checkCaller() internal view returns (bool) {
        if (
            getKernelStorage().defaultValidator.validCaller(
                msg.sender,
                msg.data
            )
        ) {
