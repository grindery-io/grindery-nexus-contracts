...
const DEPLOYMENT_NAME = "GxTonBridge";

const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  if (!hre.network.config.gxTokenAddress || !hre.network.config.gxTonBridgeOperator) {
    return true;
  }
  ...
  await deploy(DEPLOYMENT_NAME, {
    ...
    from: owner,
    args: [hre.network.config.gxTokenAddress, hre.network.config.gxTonBridgeOperator],
    log: true,
